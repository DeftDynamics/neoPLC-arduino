...
neoADS::neoADS(unsigned char _address){
  ads_address = _address;   // Set ADC i2c address to the one passed to the function
}

void neoADS::external_vref(void){
  ads_vref_int_enabled = EXT;  // Set vref trigger to specified value (Internal or external)
}

void neoADS::internal_vref(void){
  ads_vref_int_enabled = INT;  // Set vref trigger to specified value (Internal or external)
}

unsigned int neoADS::read(unsigned char channel, bool mode)
{
