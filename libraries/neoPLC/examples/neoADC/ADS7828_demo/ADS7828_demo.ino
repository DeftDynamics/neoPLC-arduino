...
ADS7828 adc(0x48);                    // Set the ADS7828 i2c address to 0x49 (A0 connected to ground, A1 connected to 5v)
...
void setup()
{
  ...
  adc.init(EXT);                         // Initialize ADC in external reference mode and initialize the i2c bus
}
...
void loop()
{
  ...
  unsigned int vref = 3300;           // Sets vref value for calculations (set to 2500 for internal reference)
